	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (choice == 1) {
				...
				if (cnt == 0) {
					System.out.println("학생 추가 후 이용가능");
				} else {
					...
					if (list[studentIdx - 1].subjects == null) {
						...
						System.out
								.println(list[studentIdx - 1].name + "학생 과목 :" + list[studentIdx - 1].subjects[0].name);
					} else {
				...
				// 만약 학생이 없다면 추가후 가능
				if (cnt == 0 || addSubjectNum == 0) {
					System.out.println("학생 추가 후, 과목 추가 후 가능 합니다.");
				} else {
					...
					if (list[studentIdx - 1].subjects == null) {
						System.out.println("과목 등록 후 이용가능");
					} else {

						int size = list[studentIdx - 1].subjects.length;

						for (int i = 0; i < size; i++) {
							System.out.println(list[studentIdx - 1].name + "학생 과목 " + (i + 1) + " ) "
									+ list[studentIdx - 1].subjects[i].name);
						}
						...
						list[studentIdx - 1].subjects[selectSub - 1].score = studentScore;
						for (int i = 0; i < size; i++) {
							System.out.println(list[studentIdx - 1].subjects[i].name + "과목 점수 :"
									+ list[studentIdx - 1].subjects[selectSub - 1].score);
						}
